...
class GnomeMenu(gobject.GObject):
	__gsignals__ = {
        'unmap': (gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, ()),
        'donthide': (gobject.SIGNAL_RUN_FIRST, gobject.TYPE_NONE, ()),
        }
	...
	def __init__(self,scan=False):
		...
		self.m.connect('expose-event',self.donthide)
	...
	def donthide(self,widget,event):
		self.emit('donthide')
	...
	def position(self,menu):
                x=180
                width,height = self.m.size_request()
                y = self.m.get_screen().get_height() - 90 - height
                push_in = True
                return (x,y,push_in)
	...
	def showmenu(self):
		...
		self.m.popup(None, None, self.position, 0,0)
		gc.collect()
